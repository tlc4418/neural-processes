...
from utils import (
     weights_init, gaussian_log_prob
)
...
class ResBlock2d(nn.Module):
    def __init__(
        ...
        for i in range(self.num_conv_layers-1):
            ...
            self.conv.append(nn.Conv2d(in_channel, in_channel, kernel_size, padding=padding, groups=in_channel, bias=add_bias))
            self.conv.append(nn.Conv2d(in_channel, in_channel, 1, bias=add_bias))
        
        self.out_norm = Normalization(in_channel)
        self.out_depthwise = nn.Conv2d(in_channel, in_channel, kernel_size, padding=padding, groups=in_channel, bias=add_bias)
...
class CNN_BLOCK(nn.Module):
    ...
    def __init__(self, n_channels, ConvBlock2d= ResBlock2d, n_blocks=3, is_channel_last=False, **kwargs):
        ...
        self.in_out_channel_pair = list(zip(in_out_channel_list[:-1],in_out_channel_list[1:]))
...
class Conv_abs(nn.Module):
    def __init__(
        ...
        self.conv=nn.Conv2d(in_channel, out_channel, kernel_size=kernel_size, padding=kernel_size//2, groups=in_channel,  bias=add_bias)
        self.reset_parameters()
...
class GridConvCNP(nn.Module):
    def __init__(
        ...
        KWARGS = dict(kernel_size=self.kernel_size_CNN,
        ...
        self.CNN = CNN_BLOCK(n_channels=self.r_dim, ConvBlock2d= ResBlock2d, n_blocks=self.n_blocks, is_channel_last=True,
                             **KWARGS)
    ...
    def forward(self, I, mask_ctx):
         ...
         loss = -gaussian_log_prob(distrib, I.permute(*([0] + list(range(2, out.dim())) + [1])))
